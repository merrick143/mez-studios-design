...
expect(record.get("state") == "integrated-production", "consumer register is not updated to the approved production integration")
expect(record.get("migrationState") == "deployed-and-human-approved", "consumer migration state is not closed")
expect(record.get("authorityBoundary", {}).get("consumerMutationAuthorised") is True, "approved consumer mutation is not recorded")
expect(record.get("authorityBoundary", {}).get("productionDeploymentAuthorised") is True, "approved deployment is not recorded")
...
expect(task.get("status") == "complete", "PORT-04 must be complete after the approved production transition")
...
print("- the registered consumer is integrated, deployed and human-approved")
print("- rc.2 remains an exact unpublished candidate without independent production authority")
