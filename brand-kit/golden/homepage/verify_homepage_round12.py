#!/usr/bin/env python3
"""Verify the GOLD-01 Round 12 card outline softening candidate."""
...
def main() -> int:
    ...
    for path in required:
        if not path.exists():
            failures.append(f"missing Round 12 artifact: {path.relative_to(BRAND_KIT.parent)}")
    if failures:
    ...
    # Round 12 change: the card outline binds to the canonical foundation
    # border token; the bespoke charcoal value is gone.
    if "--card-outline: var(--mz-border-default)" not in css:
        failures.append("card outline must bind to the canonical var(--mz-border-default) token")
    if "rgba(25, 25, 25, .42)" in css:
        failures.append("the bespoke harsh card outline value must be removed")
    if source.get("products", {}).get("cardOutlineToken") != "--mz-border-default":
        failures.append("homepage contract must record the canonical card outline token")
    ...
    for visual_contract in (
        ...
        if visual_contract not in (html + css + javascript):
            failures.append(f"carried-forward component missing in Round 12: {visual_contract}")

    # Carried-forward guards from prior rounds.
    ...
    # Round 11 consistency guard: hero must not fork the shared material overlay.
    ...
    # Round 10 fan geometry unchanged.
    for unchanged_geometry in (
        ...
        if unchanged_geometry not in css:
            failures.append(f"Round 10 fan geometry must remain unchanged: {unchanged_geometry}")
    # Round 11 centred identity unchanged.
    ...
    if products.get("heroIdentityMarkAlignment") != "centred" or products.get("heroOcclusionAuthorised") is not True:
        failures.append("Round 11 centred-identity contract must be unchanged")
    ...
    if 'candidateRevision: "golden-homepage-01-r12"' not in javascript:
        failures.append("review export does not identify the Round 12 candidate")
    ...
    if (
        ...
        or review.get("candidateRevision") != "golden-homepage-01-r12"
        or review.get("productionAuthority") is not False
    ):
        failures.append("Round 12 review record must remain pending and non-production")

    round_10 = read_json(ROUND_10_FEEDBACK)
    if round_10.get("candidateRevision") != "golden-homepage-01-r10" or len(round_10.get("sections", [])) != 1:
        failures.append("Round 10 centred-identity feedback trace is incomplete")

    round_11 = read_json(ROUND_11_FEEDBACK)
    if (
        round_11.get("candidateRevision") != "golden-homepage-01-r11"
        or round_11.get("verdict") != "round-feedback"
        or len(round_11.get("sections", [])) != 1
        or round_11.get("sections", [{}])[0].get("id") != "GH-S01"
        or "consistencyFinding" not in round_11
        or round_11.get("consistencyFinding", {}).get("priorValue") != "rgba(25, 25, 25, .42)"
    ):
        failures.append("Round 11 card-outline feedback and its consistency finding must be recorded")
...
def report(failures: list[str]) -> int:
    if failures:
        print("MEZ GOLDEN HOMEPAGE ROUND 12: FAIL")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("MEZ GOLDEN HOMEPAGE ROUND 12: PASS")
    print("- nine reviewable regions carry forward; only the card outline changed this round")
    print("- the card outline binds to the canonical var(--mz-border-default) token, not a bespoke charcoal value")
    print("- the harsh rgba(25, 25, 25, .42) outline is removed; gradient cards match the rest of the page")
    print("- Round 11 centred identity, the no-overlay-fork guard and Round 10 fan geometry are unchanged")
    print("- Living Cores stay on sized hosts with unchanged canonical-automatic motion and complete fallbacks")
    print("- the Round 11 feedback is preserved and the human gate remains open")
    return 0
