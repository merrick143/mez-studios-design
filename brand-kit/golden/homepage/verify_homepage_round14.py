#!/usr/bin/env python3
"""Verify the GOLD-01 Round 14 hero fan lift candidate."""
...
ROUND_13_FEEDBACK = ROOT / "round-13-feedback.json"
...
def main() -> int:
    ...
    for path in required:
        if not path.exists():
            failures.append(f"missing Round 14 artifact: {path.relative_to(BRAND_KIT.parent)}")
    if failures:
    ...
    for visual_contract in (
        ...
        if visual_contract not in (html + css + javascript):
            failures.append(f"carried-forward component missing in Round 14: {visual_contract}")

    # Round 14 lifted fan geometry; the Round 13 values must be gone.
    for lifted_geometry in (
        ...
        if lifted_geometry not in css:
            failures.append(f"Round 14 lifted fan geometry missing: {lifted_geometry}")
    for stale_geometry in ("clamp(22px, 2.6vw, 38px)", "clamp(-74px, -5.2vw, -52px)"):
        if stale_geometry in css:
            failures.append(f"stale Round 13 fan geometry must not remain: {stale_geometry}")
    ...
    if 'candidateRevision: "golden-homepage-01-r14"' not in javascript:
        failures.append("review export does not identify the Round 14 candidate")
    ...
    if (
        ...
        or review.get("candidateRevision") != "golden-homepage-01-r14"
        or review.get("productionAuthority") is not False
    ):
        failures.append("Round 14 review record must remain pending and non-production")

    round_12 = read_json(ROUND_12_FEEDBACK)
    if round_12.get("candidateRevision") != "golden-homepage-01-r12" or len(round_12.get("sections", [])) != 1:
        failures.append("Round 12 further-compaction feedback trace is incomplete")

    round_13 = read_json(ROUND_13_FEEDBACK)
    if (
        round_13.get("candidateRevision") != "golden-homepage-01-r13"
        or round_13.get("verdict") != "round-feedback"
        or len(round_13.get("sections", [])) != 1
        or round_13.get("sections", [{}])[0].get("id") != "GH-S01"
        or "higher" not in round_13.get("sections", [{}])[0].get("feedback", "").lower()
    ):
        failures.append("Round 13 hero-lift feedback must be recorded")
...
def report(failures: list[str]) -> int:
    if failures:
        print("MEZ GOLDEN HOMEPAGE ROUND 14: FAIL")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("MEZ GOLDEN HOMEPAGE ROUND 14: PASS")
    print("- nine reviewable regions carry forward; only the hero fan geometry lifted this round")
    print("- side cards sit higher (flatter arc) with only a small overlap increase")
    print("- the Round 13 fan geometry is replaced; centred identity, outline token and no-overlay-fork guard hold")
    ...
    print("- the Round 13 feedback is preserved and the human gate remains open")
    return 0
