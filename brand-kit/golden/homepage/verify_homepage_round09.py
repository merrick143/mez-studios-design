#!/usr/bin/env python3
"""Verify the GOLD-01 Round 09 hero copy trim candidate."""
...
def main() -> int:
    ...
    for path in required:
        if not path.exists():
            failures.append(f"missing Round 09 artifact: {path.relative_to(BRAND_KIT.parent)}")
    if failures:
    ...
    # Round 09 removals: the family-caption divider and the hero supporting
    # line must be gone from markup, styling and rendered text.
    if "family-caption" in (html + css):
        failures.append("family-caption divider and its CSS must be removed")
    if "supporting-line" in (html + css):
        failures.append("supporting-line class and its CSS must be removed")
    if "AI OS is available now" in html:
        failures.append("the removed hero supporting line must not return")
    if "One operating layer" in html or "Five focused systems" in html:
        failures.append("the removed family-caption labels must not return")
    ...
    # Every other Round 08 contract carries forward unchanged.
    for visual_contract in (
        ...
        if visual_contract not in (html + css + javascript):
            failures.append(f"Round 08 component carried forward but missing in Round 09: {visual_contract}")
    ...
    if products.get("heroSupportingLine") is not False or products.get("heroFamilyCaption") is not False:
        failures.append("homepage contract must record both removals as false")
    if products.get("heroArrangement") != "centred-spread-deck-2-1-2-compact":
        failures.append("hero arrangement must be unchanged from Round 08")
    if products.get("heroIdentityMarkScope") != "hero-local":
        failures.append("hero identity mark scope must be unchanged from Round 08")
    ...
    if 'candidateRevision: "golden-homepage-01-r09"' not in javascript:
        failures.append("review export does not identify the Round 09 candidate")
    ...
    if (
        ...
        or review.get("candidateRevision") != "golden-homepage-01-r09"
        or review.get("productionAuthority") is not False
    ):
        failures.append("Round 09 review record must remain pending and non-production")

    round_07 = read_json(ROUND_07_FEEDBACK)
    if round_07.get("candidateRevision") != "golden-homepage-01-r07" or len(round_07.get("sections", [])) != 1:
        failures.append("Round 07 hero-only feedback trace is incomplete")

    round_08 = read_json(ROUND_08_FEEDBACK)
    if (
        round_08.get("candidateRevision") != "golden-homepage-01-r08"
        or round_08.get("verdict") != "round-feedback"
        or len(round_08.get("sections", [])) != 1
        or round_08.get("sections", [{}])[0].get("id") != "GH-S01"
        or "family-caption" not in round_08.get("sections", [{}])[0].get("feedback", "").lower().replace(" ", "-")
        and "divider" not in round_08.get("sections", [{}])[0].get("feedback", "").lower()
    ):
        failures.append("Round 08 hero-copy-trim feedback must be recorded")
...
def report(failures: list[str]) -> int:
    if failures:
        print("MEZ GOLDEN HOMEPAGE ROUND 09: FAIL")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("MEZ GOLDEN HOMEPAGE ROUND 09: PASS")
    print("- nine reviewable regions carry forward; only the hero copy changed this round")
    print("- the family-caption divider and hero supporting line are fully removed, markup and CSS")
    print("- the compacted Round 08 deck geometry and hero-scoped identity mark are unchanged")
    ...
    print("- the Round 08 feedback is preserved and the human gate remains open")
    return 0
