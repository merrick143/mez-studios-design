const decisions = [
  ["overallDirection", "Overall direction", "Does this feel like a credible beginning for the Mez Systems homepage?"],
  ["messageHierarchy", "Message hierarchy", "Do the centred title, support and actions land in the right order and scale?"],
  ["productFamilyStage", "Product family stage", "Do five aligned cores communicate a real family without becoming a card grid?"],
  ["desktopOpening", "Desktop opening", "Does the desktop viewport feel intentional, premium and recognisably Mez?"],
  ["mobileOpening", "Mobile opening", "Does mobile preserve the idea, actions and full product family without feeling cramped?"],
];
...
document.querySelector("[data-review-form]").addEventListener("submit", async (event) => {
  ...
  const record = {
    schemaVersion: "1.0.0",
    studyId: "MEZ-HERO-FIRST-VIEWPORT-02",
  ...
  try {
    ...
    console.error("[mez-hero-02] Clipboard export failed.", error);
