...
/* Motion allocation for the lab.
   Five candidate bars, one Living Core. A single renderer moves to whichever bar
   ...
   only differs in having five candidates instead of three.
...
function setBadge(anchor, state) {
  const badge = badges.get(anchor?.dataset.liveAnchor?.replace("v", ""));
  if (!badge) return;
